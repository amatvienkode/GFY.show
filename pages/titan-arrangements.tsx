...
export interface TitanArrangement {
  titanName: string;
  issuerName: string;
...
interface Props {
  arrangements?: TitanArrangement[];
  error?: string;
}

// Helper function to read and parse markdown files from titan_arrangements directory
const getTitanArrangements = (): TitanArrangement[] => {
  const contentDir = path.join(process.cwd(), "content", "titan_arrangements");
  try {
    if (!fs.existsSync(contentDir)) {
      console.warn(
        `Content directory not found for titan_arrangements: ${contentDir}`
      );
      return [];
    }
    ...
    const arrangements = filenames
      ...
      .map((filename) => {
        ...
        return data as TitanArrangement;
      });
    return arrangements;
  } catch (error) {
    console.warn(
      `Could not read content for titan_arrangements:`,
...
export const getStaticProps: GetStaticProps = async () => {
  try {
    const arrangements = getTitanArrangements();
    ...
    console.error("Error in getStaticProps reading titan arrangements:", error);
    return {
      props: {
        error:
...
const TitanArrangementsPage = ({ arrangements, error }: Props) => {
  if (error) {
    return (
      ...
      <section className="flex flex-col items-center px-4 sm:px-8 pt-4 pb-8 justify-center">
        <div className="bg-white max-w-5xl rounded-lg px-4 sm:px-16 pb-8">
          ...
          <h1 className="text-2xl font-roboto balance text-center mb-6">
            Titan Compensation Arrangements; Potential Conflicts of Interest
          </h1>

          <p className=" mb-6 leading-relaxed">
            The following information discloses the financial arrangements
            between each Titan and the show, as well as, where applicable, each
            Titan and the people and/or companies who appear on the show. These
            arrangements consist of compensated arrangements and ownership of
            securities, which could potentially give rise to conflicts of
            interest.
          </p>
              ...
              {arrangements.map((arrangement, index) => (
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                    <div>
                      <span className="font-bold">Titan Name:</span>
                      <p className="mt-1">{arrangement.titanName}</p>
                    </div>
                    <div>
...
export default TitanArrangementsPage;
